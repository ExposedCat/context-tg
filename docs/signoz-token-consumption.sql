...
SELECT
    ...
    multiIf(
        token_kind = 'uncached_input', 'Uncached input',
        token_kind = 'cached_input', 'Cached input',
        'Output'
    ) AS __name__,
    toFloat64(sum(multiIf(
        token_kind = 'uncached_input',
            attributes_number['input_tokens'] - attributes_number['cached_tokens'],
        token_kind = 'cached_input', attributes_number['cached_tokens'],
        attributes_number['output_tokens']
    ))) AS value
FROM signoz_logs.distributed_logs_v2
ARRAY JOIN ['uncached_input', 'cached_input', 'output'] AS token_kind
...
GROUP BY dateTrunc($bucket, fromUnixTimestamp64Nano(timestamp)), token_kind
